...
NumericMatrix domainInterest_s, NumericMatrix activation_s, NumericVector control){
  ...
  arma::mat activation(activation_s.begin(), control[0], control[2], false);
...
//                }
                ...
                data(count, 2) = domainInterest(topic-1, domain-1) * activation_s(day-1, topic-1);
                data(count, 3) = domain;
                data(count, 4) = day;
                data(count, 5) = topic;
                data(count, 6) = activation(day-1, topic-1);
                count++;
